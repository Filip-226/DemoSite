	...
	if(current_user_can('level_10')) { ?>
		<div class="sidebar-expire-notice center">
			...
			<form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
				...
				<div class="clear5"></div>
				<input type="submit" name="submit" class="greenbutton rad3" value="<?php _e('Activate ad','escortwp'); ?>" />
				<div class="clear5"></div>
			</form>
		</div>
	<?php
	...
	if(current_user_can('level_10')) { ?>
		<div class="sidebar-expire-notice center">
			...
			<form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
				...
				<div class="clear5"></div>
				<input type="submit" name="submit" class="greenbutton rad3" value="<?php _e('Activate profile','escortwp'); ?>" />
				<div class="clear5"></div>
			</form>
		</div>
	<?php
...
if (is_single() && get_post_meta(get_the_ID(), "needs_payment", true) == "1" && current_user_can('level_10')) { ?>
	<div class="sidebar-expire-notice center">
		<?php _e('The owner of this profile has not paid the registration fee yet','escortwp'); ?>
		<div class="clear5"></div>
		<form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
			<input type="hidden" name="action" value="activateunpaidprofile" />
			<div class="clear20"></div>
			<?=__('Activate account for a period of','escortwp')?>:
			<div class="clear5"></div>
			...
			<div class="clear5"></div>
			<input type="submit" name="submit" class="activation-button greenbutton rad3" value="<?php _e('Activate profile','escortwp'); ?>" />
			<div class="clear5"></div>
		</form>
	</div>
	<?php
...
if(is_single() && get_the_author_meta('ID') == $userid && get_post_type() == $taxonomy_profile_url && !get_post_meta(get_the_ID(), "needs_payment", true) && 
	...
	if(payment_plans('premium','price') && get_post_meta(get_the_ID(), "premium", true) == "0") { ?>
		<div class="orangebutton buypremium"><?php _e('Buy Premium Position','escortwp'); ?><span class="rad3 greendegrade"><?php echo format_price('premium','small') ?></span></div>
	    <div class="buypremium_details blueishdegrade">
		    <?php closebtn('2') ?>
		    <?php
	    	printf(esc_html__('Premium %1$s will always be shown first and before any normal %2$s, in all the pages of the site.','escortwp'),$taxonomy_profile_name_plural,$taxonomy_profile_name_plural);
	...
	if (payment_plans('featured','price') && get_post_meta(get_the_ID(), "featured", true) != "1") { ?>
		<div class="pinkbutton buyfeatured"><?php _e('Buy Featured Position','escortwp'); ?><span class="rad3 greendegrade"><?php echo format_price('featured','small') ?></span></div>
	    <div class="buyfeatured_details blueishdegrade">
		    <?php closebtn('2') ?>
		    <?php _e('After you buy a featured position you will be placed in the header slider for maximum visibility.','escortwp'); ?> <?php _e('Only the latest','escortwp'); ?> <?php echo get_option("headerslideritems"); ?> <?php printf(esc_html__('%s will be shown at one time.','escortwp'),$taxonomy_profile_name_plural); ?>
			<?php
...
if($userstatus == "member" && !get_user_meta($userid, "vip", true) && payment_plans('vip','price')) {
	echo '<div class="sidebar-expire-notice center">';
		...
		echo '<div class="clear20"></div>';
		echo '<div class="center">'.generate_payment_buttons("vip", $userid, 'Upgrade to VIP')."</div> <!--center-->";
		echo '<div class="clear5"></div>';
	...
	<div class="dropdownlinks dropdownlinks-dropdown my-account-links">
        ...
        <ul>
			...
			<?php if($agency_has_not_payed == "yes") { ?>
				<li><?php _e('Other profile links will be shown after payment','escortwp'); ?></li>
			<?php } else { ?>
	...
	<div class="dropdownlinks dropdownlinks-dropdown dropdownlinks-profile">
		...
		<?php } else { ?>
			<?php if(get_option("hide8") != "1") { ?>
				<li><a href="<?php echo get_permalink(get_option('escort_tours_page_id')); ?>"><span class="icon icon-suitcase"></span> <?php _e('Manage my Tours','escortwp'); ?></a></li>
			<?php } ?>
...
if ($userstatus == "member" && !get_user_meta( $current_user->ID, "emailhash", true )) { ?>
	<div class="dropdownlinks dropdownlinks-dropdown">
        ...
        <ul>
			<li><a href="<?php echo get_permalink(get_option('member_favorite_escorts_page_id')); ?>"><span class="icon icon-heart"></span> <?php printf(esc_html__('My Favourite %s','escortwp'),ucwords($taxonomy_profile_name_plural)); ?></a></li>
...
if (is_single() && get_post_type() == $taxonomy_profile_url && ((get_the_author_meta('ID') == $userid && $userstatus == $taxonomy_agency_url) || current_user_can('level_10'))) {
	...
	<div class="agencyeditbuttons dropdownlinks dropdownlinks-dropdown">
		...
		<?php } else { ?>
			...
			<?php if(get_option("hide8") != "1") { ?>
	    	<li><a class="addtours"><span class="icon icon-suitcase"></span> <?php _e('Add Tours','escortwp'); ?></a></li>
	    	<?php } ?>
			<?php if(get_option("hide7") != "1") { ?>
	            ...
	            <form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
					...
					<div class="upgradeescortparent">
						<div class="upgradebuttons">
							<div class="upgradebutton pinkbutton rad3 l"><?php _e('Premium','escortwp'); ?></div>
							<?php if ($premium_status == "1") { ?>
								<input type="submit" name="delpremium" value="X" class="rad3 redbutton r" />
							<?php } ?>
						</div>
						<div class="upgradeescortbox rad3">
							...
							<div class="clear10"></div>
							<div class="center">
								<input type="submit" name="premium" value="<?php if ($premium_status == "1") { echo __('Extend','escortwp')." "; } else { echo __('Add','escortwp')." "; } ?><?php _e('Premium','escortwp'); ?>" class="greenbutton ok-button rad3" />
							<?php if ($premium_status == "1") { ?>
								<div class="clear10"></div>
								<input type="submit" name="delpremium" value="<?=__('Delete premium','escortwp')?>" class="rad3 del-button redbutton" />
							<?php } ?>
							</div> <!--center-->
							<div class="clear"></div>
						</div> <!-- premium -->
					...
					<div class="upgradeescortparent">
						<div class="upgradebuttons">
							<div class="upgradebutton pinkbutton rad3 l"><?php _e('Featured','escortwp'); ?></div>
						<?php if ($featured_status == "1") { ?>
							<input type="submit" name="delfeatured" value="X" class="rad3 redbutton r" />
						<?php } ?>
						</div>
						<div class="upgradeescortbox rad3">
							<?php closebtn('2') ?>
							...
							<div class="clear10"></div>
							<div class="center">
							<input type="submit" name="featured" value="<?php if ($featured_status == "1") { echo __('Extend','escortwp')." "; } else { echo __('Add','escortwp')." "; } ?><?php _e('Featured','escortwp'); ?>" class="greenbutton ok-button rad3" />
							<?php if ($featured_status == "1") { ?>
								<div class="clear10"></div>
								<input type="submit" name="delfeatured" value="<?=__('Delete Featured','escortwp')?>" class="rad3 del-button redbutton" />
							<?php } ?>
							</div> <!--center-->
							<div class="clear"></div>
						</div> <!-- featured -->
					...
					<div class="upgradeescortparent">
						<div class="upgradebuttons">
							<div class="upgradebutton pinkbutton rad3 l"><?php _e('Profile expiration','escortwp'); ?></div>
						<?php if ($expiration_status) { ?>
							<input type="submit" name="delexpiration" value="X" class="rad3 redbutton r" />
						<?php } ?>
						</div>
						<div class="upgradeescortbox rad3">
							<?php closebtn('2') ?>
							...
							<div class="clear10"></div>
							<div class="center">
							<input type="submit" name="expirationperiod" value="<?php if ($expiration_status) { echo __('Extend','escortwp')." "; } else { echo __('Add','escortwp')." "; } ?><?php _e('Expiration','escortwp'); ?>" class="greenbutton ok-button rad3" />
							<div class="clear10"></div>
							<input type="submit" name="delexpiration" value="<?=__('Set as expired','escortwp')?>" class="rad3 del-button redbutton" />
							</div> <!--center-->
							<div class="clear"></div>
					...
					<input type="submit" name="verified" value="<?php if (get_post_meta(get_the_ID(), "verified", true) == "1") { echo __('Remove','escortwp')." "; } else { echo __('Mark as','escortwp')." "; } ?><?php _e('Verified','escortwp'); ?>" class="pinkbutton rad3 l<?php if (get_post_meta(get_the_ID(), "verified", true) == "1") { echo " redbutton"; } ?>" /><div class="clear5"></div>
				</form>
            </li>
		<?php } // if is admin ?>
		<?php if(!get_post_meta(get_option('escortpostid'.$userid), 'notactive', true)) { ?>
			<li>
				...
				<form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
					<input type="hidden" name="action" value="settoprivate" />
					<input type="submit" name="submit" value="<?=$button_text?>" class="pinkbutton rad3<?php if(get_post_status(get_the_ID()) == "publish") echo " redbutton"; ?>" />
				</form>
				<div class="clear10"></div>
			</li>
		<?php } ?>
			<li><a class="delete"><span class="redbutton rad3"><?php _e('Delete','escortwp'); ?></span></a></li>
        </ul>
        <div class="clear"></div>
...
if (current_user_can('level_10')) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$('.admineditbuttons .addlogo').on('click', function(){
					$('.bodybox').hide();
					$('.agency_options_add_logo').slideDown('fast');
				});
	        ...
	        <ul>
		            ...
		            <form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
						...
						<div class="upgradeescortparent">
							<div class="upgradebuttons">
								<div class="upgradebutton pinkbutton rad3 l"><?php _e('Profile expiration','escortwp'); ?></div>
							<?php if ($expiration_status) { ?>
								<input type="submit" name="delexpiration" value="X" class="pinkbutton rad3 redbutton r delbtn" />
							<?php } ?>
							</div>
							<div class="upgradeescortbox rad3">
								<?php closebtn('2') ?>
								...
								<div class="clear10"></div>
								<input type="submit" name="expirationperiod" value="<?php if ($expiration_status) { echo __('Extend','escortwp')." "; } else { echo __('Add','escortwp')." "; } ?><?php _e('Expiration','escortwp'); ?>" class="pinkbutton rad3 l" />
								<?php if ($expiration_status) { ?>
									<input type="submit" name="delexpiration" value="X" class="pinkbutton rad3 redbutton r" />
								<?php } ?>
								<div class="clear"></div>
							</div> <!-- registration-expiration -->
						</div> <!-- upgradeescortparent registration-expiration -->
		    	...
		    	<li>
		            <form action="<?php echo get_permalink(get_the_ID()); ?>" method="post">
						...
						<input type="submit" name="submit" value="<?=__('Needs payment','escortwp')?>" class="pinkbutton rad3 redbutton" />
						<div class="clear10"></div>
					</form>
		    	</li>
		    	<li><a class="deleteprofile"><span class="redbutton rad3 l"><?php _e('Delete','escortwp'); ?></span></a></li>
		    </ul>
	        <div class="clear"></div>
		</div> <!-- AGENCY EDIT BUTTONS -->
	    <div class="clear"></div>
        	...
        	<li>
				...
				<div class="upgradeescortbox manuallyactivatevip_div ok rad5">
					...
					<form action="" method="post">
						...
						<div class="clear10"></div>
						<input type="submit" name="addvip" class="greenbutton ok-button rad3" value="<?php _e('Add VIP user','escortwp'); ?>" />
						<div class="clear10"></div>
						<input type="submit" name="removevip" class="redbutton del-button rad3" value="<?php _e('Remove VIP','escortwp'); ?>" />
					</form>
				</div> <!-- manually activate VIP -->
        	</li>
        	<li>
        		<a class="manuallyactivatetour"><span class="icon icon-suitcase"></span> <?php _e('Activate Tour','escortwp'); ?></a>
				<div class="upgradeescortbox manuallyactivatetour_div ok rad5">
					...
					<form action="" method="post">
						...
						<div class="clear10"></div>
						<input type="submit" name="submit" class="greenbutton ok-button rad3" value="<?php _e('Activate tour','escortwp'); ?>" />
					</form>
	...
	<?php if((get_post_type() == $taxonomy_profile_url || get_post_type() == $taxonomy_agency_url || get_post_type() == "ad" || get_post_type() == "review") && is_single()) { ?>
		<div class="dropdownlinks dropdownlinks-userid">
			...
			<?php } ?>
			<div class="clear5"></div>
			<div class="center"><a class="edit-user" href="<?=get_admin_url('', 'user-edit.php?user_id='.get_the_author_meta('ID'))?>"><?php _e('Edit user','escortwp'); ?></a></div>
	        <div class="clear"></div>
		</div>
        <div class="clear"></div>
	<?php } // if is_single() ?>
...
if(get_post_type() == $taxonomy_profile_url && get_the_author_meta('ID') == $userid) {
	...
	if($agency_expire_date) {
		...
		if(get_post_meta($agency_profile_id, "agency_renew", true)) {
		    ...
		    echo '<div class="clear20"></div>';
			echo '<div class="center">'.generate_payment_buttons('agreg',$agency_profile_id,__('Extend registration','escortwp')).'</div> <!--center-->';
		    echo '<div class="clear5"></div>';
			echo '<small>'.format_price('agreg').'</small>';
		}
	...
	if(get_post_meta(get_the_ID(), "escort_expire", true)) {
		...
		if(!current_user_can('level_10')) {
			if(get_post_meta(get_the_ID(), "escort_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('agescortreg',get_the_ID(),__('Extend registration','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
				echo '<div class="center"><small>'.format_price('agescortreg').'</small></div> <!--center-->';
			}
		}
		echo '</div>';
	}
	...
	if(get_post_meta(get_the_ID(), "premium", true) == "1") {
		...
		if(!current_user_can('level_10')) {
			if(get_post_meta(get_the_ID(), "premium_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('premium',get_the_ID(),__('Extend premium','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
	...
	if(get_post_meta(get_the_ID(), "featured", true) == "1") {
		...
		if(!current_user_can('level_10')) {
			if(get_post_meta(get_the_ID(), "featured_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('featured',get_the_ID(),__('Extend featured','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
	...
	if(get_post_meta($independent_profile_id, "escort_expire", true)) {
		...
		echo '<div class="sidebar-expire-notice sidebar-expire-notice-has-mobile pinkdegrade center" data-payment-plan="reg">';
			...
			if(get_post_meta($independent_profile_id, "escort_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('indescreg',$independent_profile_id,__('Extend registration','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
				echo '<div class="center"><small>'.format_price('indescreg').'</small></div> <!--center-->';
			}
		echo '</div>';
	}
	// show registration expiration
	...
	if(get_post_meta($independent_profile_id, "premium", true) == "1") {
		...
		echo '<div class="sidebar-expire-notice sidebar-expire-notice-has-mobile orangedegrade center" data-payment-plan="premium">';
			...
			if(get_post_meta($independent_profile_id, "premium_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('premium',$independent_profile_id,__('Extend premium','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
				echo '<small>'.format_price('premium').'</small>';
			}
		echo '</div>';
	...
	if(get_post_meta($independent_profile_id, "featured", true) == "1") {
		...
		echo '<div class="sidebar-expire-notice sidebar-expire-notice-has-mobile bluedegrade center" data-payment-plan="featured">';
			...
			if(get_post_meta($independent_profile_id, "featured_renew", true)) {
			    ...
			    echo '<div class="clear20"></div>';
				echo '<div class="center">'.generate_payment_buttons('featured',$independent_profile_id,__('Extend featured','escortwp')).'</div> <!--center-->';
			    echo '<div class="clear5"></div>';
				echo '<small>'.format_price('featured').'</small>';
			}
		echo '</div>';
...
if(is_user_logged_in() && get_user_meta($userid, 'vip', true)) {
	if(get_user_meta($userid, 'vip_expire', true)) {
		...
		$vip_mobile_expire_text = __('VIP expiration:','escortwp').' <b>'.$featured_expire_date.'</b>';
	} else {
		$vip_expire_date = strtolower(__('forever','escortwp'));
		$vip_mobile_expire_text = __('VIP status is active <b>forever</b>','escortwp');
	}
	echo '<div class="sidebar-expire-notice-mobile bluedegrade text-center" data-payment-plan="featured">';
		echo '<div class="expiration-date">'.$vip_mobile_expire_text.'</div>';
	...
	echo '<small>'.__('Your VIP status is active until','escortwp').':</small><b>'.$vip_expire_date.'</b>';

	if(get_user_meta($userid, "vip_expire", true) && payment_plans('vip','price')) {
		echo '<div class="clear20"></div>';
		echo '<div class="center">'.generate_payment_buttons("vip", $userid, 'Extend VIP Status')."</div> <!--center-->";
		echo '<div class="clear5"></div>';
		echo '<small>'.format_price('vip').'</small>';
	}
	echo '</div>';
...
?>
	<div class="quicksearch">
    	...
    	<form action="<?php echo get_permalink(get_option('search_page_id')); ?>" method="post" class="form-styling">
			...
			<?php
			if($_POST['country'] > 0) { $temp_country = $_POST['country']; } else { $temp_country = 0; } 
			if($_POST['gender'] > 0) { $temp_gender = $_POST['gender']; } else { $temp_gender = 0; } 			
			$args = array(
			...
			<div class="search-cities-input form-input col100"></div>
			 ...
			 <div class="form-input col100">
                <label style="width: 100%;display: block;margin-bottom: 4px;">Age</label>
                  	<input type="number" name="min_age" size="4" id="min_age" style="width: 50%;float: left;" placeholder="From" class="input smallinput text-center" value="<?php echo $_POST['min_age']; ?>"> 
                    <input type="number" name="max_age" size="4" id="max_age" style="width: 50%;" placeholder="To" class="input smallinput text-center" value="<?php echo $_POST['max_age']; ?>"> 
                  	<div style="clear:both;"></div>
			</div>		
			
            <div class="form-input col100">
                <label style="width: 100%;display: block;margin-bottom: 4px;">Height (Ft & In)</label>
                  	<option value=""><?php _e('Select','escortwp'); ?></option>
	<?php foreach($height_a as $key=>$b) { ?>
	<?php } ?>
  ...
  <option value=""><?php _e('Select','escortwp'); ?></option>
	...
	<option value="5.2">5'2</option> 
	 ...
	 <option value="5.11">5'11</option>
            ...
            <div class="form-input col100">
                <label style="width: 100%;display: block;margin-bottom: 4px;">Weight (Lbs)</label>
                  	<input type="number" name="min_weight" size="4" id="min_weight" style="width: 50%;float: left;" placeholder="From" class="input smallinput text-center" value="<?php echo $_POST['min_weight']; ?>"> 
                    <input type="number" name="max_weight" size="4" id="max_weight" style="width: 50%;" placeholder="To" class="input smallinput text-center" value="<?php echo $_POST['max_weight']; ?>"> 
                  	<div style="clear:both;"></div>
			</div>				
