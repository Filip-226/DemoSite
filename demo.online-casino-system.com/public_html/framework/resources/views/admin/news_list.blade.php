...
@section('content')
							...
							<form class="form-horizontal" role="form" method="GET" action="{{ url('/panel/admin/news/actions') }}" id="news_action_form" name="news_action_form" data-url="{{ url('/') }}" >
												...
												<div class="panel-body no-padding table-responsive">
													<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered">
														...
														<tr> 
															<td></td>
															<td>
																<input type="text" class="form-control" id="newstitle_search_txt" name="newstitle_search_txt" value="{{ Input::get('newstitle_search_txt') }}" placeholder="Title">
															</td>
															<td>
																<input type="submit" class="btn btn-primary pull-left" value="Search" id="search_btn" name="search_btn" class="btn-success btn" />
															</td>
														</tr>
														<?php $i=1; ?>
														@foreach($news_list as $news)
															<tr> 
																...
																<td>{{$news->title}}</td>
																<td>
																	<a href="{{URL::to('/panel/admin/news/view/' . $news->news_id)}}">{{ Lang::get('general.view_action') }}</a> | 
																	<a href="{{URL::to('/panel/admin/news/edit/' . $news->news_id)}}">{{ Lang::get('general.edit_action') }}</a> | 
																	<a href="{{URL::to('/panel/admin/news/delete/' . $news->news_id)}}" onclick="return confirm('{{ Lang::get('news.delete_msg') }} ({{$news->title}})?');">{{ Lang::get('general.delete_action') }}</a>
																</td>
															</tr>
															<?php $i++; ?>
														@endforeach
