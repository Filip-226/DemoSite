										...
										<div class="col-md-12">
											<div class="panel panel-default">
												<div class="panel-heading">
													...
													<div class="panel-ctrls"><a href="{{ URL::route('user.inbox_compose') }}" class="btn-success btn">Compose</a></div>
												</div>
													...
													<table cellpadding="0" cellspacing="0" width="100%" class="table table-striped table-bordered">
														<thead>
															<tr>
																...
																<th width="10%">{{ Lang::get('general.action') }}</th>
															</tr>
														</thead>
														<tbody>
														<?php $i=1; ?>
														@foreach($inbox_list as $inbox)
															<tr> 
																...
																<a href="{{ URL::route('user.inbox_view', array($inbox->inbox_id)) }}"><b>{{$inbox->inbox_subject}}</b></a>
																@else
																<a href="{{ URL::route('user.inbox_view', array($inbox->inbox_id)) }}">{{$inbox->inbox_subject}}</a>
																@endif
																</td>
																<td><a href="{{ URL::route('user.inbox_delete', array($inbox->inbox_id)) }}">{{ Lang::get('general.delete_action') }}</a></td>
															</tr>
															<?php $i++; ?>
														@endforeach
														</tbody>
