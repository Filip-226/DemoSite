...
class Page
{
	...
	function __construct($theme="", $module="", $template="") {
		global $THEMES;

		if (!$theme)
			$theme = ($_SESSION['auth']) ? SITE_THEME_USER : SITE_THEME_GUEST;
		$db = debug_backtrace();
		$dirname = explode('/', dirname($db[0]['file']));
		$template = ($template) ? $template : "index";
		$module = ($module) ? $module : array_pop($dirname);
		$t = $THEMES[$theme];
		foreach($t as $k => $v) {
			$this->assigned[$k]['file'] = "templates/$v.html";
			$this->assigned[$k]['vars'] = [];
		}
		$this->assigned['A.page']['file'] = "modules/".$module."/".$template.".html";
		$this->assigned['A.page']['vars'] = [];
		$this->html = "";
	}
